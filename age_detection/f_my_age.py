...
3. input an image where only one face is visible (use model face detection to extract an image with only the face)
...
class Age_Model():
    ...
    def loadModel(self):
        ...
        ''' if os.path.isfile(home+'/.deepface/weights/age_model_weights.h5') != True:
            ...
            url = 'https://drive.google.com/file/d/1y6t4NspKpqTO6tcVP5y7VVWYI1DhcOfp/view?usp=sharing' 
            output = home+'/.deepface/weights/age_model_weights.h5'
            gdown.download(url, output, quiet=False)
        '''
        age_model.load_weights('D:\Face-Emotion-and-Info-Detection/age_detection/age_model_weights.h5')
        return age_model
